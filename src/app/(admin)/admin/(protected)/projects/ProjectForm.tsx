...
export function ProjectForm({ project, action }: ProjectFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 max-w-2xl">
      ...
      <FormField label="Tagline" name="tagline" required hint="One sentence — shown on project cards">
        <input
          id="tagline"
          name="tagline"
      ...
      <div className="grid grid-cols-3 gap-4">
        <FormField label="Status" name="status">
          <select id="status" name="status" defaultValue={project?.status ?? 'featured'} className={selectClass}>
